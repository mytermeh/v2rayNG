...
//        if (!result.status) return
    ...
    private const val SINGLE_TEST_TIMEOUT_SEC = 15L

    /**
     * Measures the connection delay for the current V2Ray configuration.
     * Tests with primary URL first, then falls back to alternative URL if needed.
     * Each measurement has a hard timeout to prevent indefinite blocking.
    ...
    private fun measureV2rayDelay() {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            ...
            try {
                time = measureDelayWithTimeout(SettingsManager.getDelayTestUrl())
            } catch (e: Exception) {
            ...
            if (time == -1L) {
                try {
                    time = measureDelayWithTimeout(SettingsManager.getDelayTestUrl(true))
                } catch (e: Exception) {
    ...
    /**
     * Wraps coreController.measureDelay with a hard timeout to prevent indefinite blocking.
     * Returns -1 if the measurement times out.
     */
    private fun measureDelayWithTimeout(url: String): Long {
        val executor = java.util.concurrent.Executors.newSingleThreadExecutor()
        return try {
            val future = executor.submit(java.util.concurrent.Callable {
                coreController.measureDelay(url)
            })
            future.get(SINGLE_TEST_TIMEOUT_SEC, java.util.concurrent.TimeUnit.SECONDS)
