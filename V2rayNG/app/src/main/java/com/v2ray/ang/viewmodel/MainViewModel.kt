...
//            }
    ...
    fun getSubscriptions(context: Context): List<GroupMapItem> {
        ...
        subscriptions.forEach { sub ->
            // Hide empty Default tab when other subscriptions exist
    ...
    fun onTestsFinished() {
        viewModelScope.launch(Dispatchers.Default) {
            ...
            // Auto-select the best server (lowest positive ping)
            val shouldAutoSelect = isQuickTest
            if (shouldAutoSelect) {
                selectBestServer()
            }
    ...
    /**
     * Selects the server with the lowest positive ping from the current server list.
     */
