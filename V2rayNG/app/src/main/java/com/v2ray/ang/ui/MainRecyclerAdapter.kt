...
class MainRecyclerAdapter(
    ...
    override fun onBindViewHolder(holder: BaseViewHolder, position: Int) {
        if (holder is MainViewHolder) {
            ...
            //layoutIndicator + selected highlight
            val isSelected = guid == MmkvManager.getSelectServer()
            if (isSelected) {
                holder.itemMainBinding.layoutIndicator.setBackgroundResource(R.color.colorIndicator)
                holder.itemMainBinding.tvName.setTypeface(null, Typeface.BOLD)
                holder.itemView.setBackgroundColor(0x08000000) // ~3% black = very subtle gray
            } else {
                holder.itemMainBinding.layoutIndicator.setBackgroundResource(0)
                holder.itemMainBinding.tvName.setTypeface(null, Typeface.NORMAL)
                holder.itemView.setBackgroundColor(Color.TRANSPARENT)
            }

            //capability indicators
            holder.itemMainBinding.tvCapMux.visibility =
                if (MuxFragmentBenchmarkManager.supportsMux(profile)) View.VISIBLE else View.GONE
            holder.itemMainBinding.tvCapFrag.visibility =
                if (MuxFragmentBenchmarkManager.supportsFragment(profile)) View.VISIBLE else View.GONE
            ...
            //layout - always use compact mode with ⋮ menu
            holder.itemMainBinding.layoutMore.setOnClickListener { view ->
                adapterListener?.onShare(guid, profile, position, view)
            }
