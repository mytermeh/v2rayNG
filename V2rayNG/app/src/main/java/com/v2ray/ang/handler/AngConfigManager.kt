...
object AngConfigManager {
    ...
    /**
     * Extracts valid subscription URLs from text.
     */
    fun extractSubscriptionUrls(server: String?): List<String> {
        if (server == null) return emptyList()
        val urls = mutableListOf<String>()
        val configSchemes = EConfigType.entries.map { it.protocolScheme } + HY2
        server.lines().distinct().forEach { line ->
            val trimmed = line.trim()
            // Skip lines that look like individual config links (vmess://, vless://, etc.)
            if (configSchemes.any { trimmed.startsWith(it, ignoreCase = true) }) return@forEach
            if (Utils.isValidSubUrl(trimmed)) {
                urls.add(trimmed)
            }
        }
        return urls
    ...
    /**
     * Checks if a subscription URL already exists.
     * @return The existing subscription GUID if duplicate, null otherwise.
     */
    fun findExistingSubscriptionByUrl(url: String): String? {
        return MmkvManager.decodeSubscriptions().firstOrNull { it.subscription.url == url }?.guid
    }

    /**
     * Imports a subscription with user-configured settings.
     * @return The new subscription GUID.
     */
    fun importSubscription(subItem: SubscriptionItem): String {
        return MmkvManager.encodeSubscription("", subItem)
    }

    /**
     * Updates a single subscription by GUID.
     * @return The number of configs fetched.
     */
    fun updateConfigViaSingleSub(guid: String): Int {
        val subItem = MmkvManager.decodeSubscription(guid) ?: return 0
        return updateConfigViaSub(SubscriptionCache(guid, subItem))
    }

    /**
     * Extracts a meaningful name from a subscription URL.
     */
    fun extractSubscriptionName(url: String): String {
        return try {
            val uri = URI(Utils.fixIllegalUrl(url))
